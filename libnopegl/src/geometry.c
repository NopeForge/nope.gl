                      ...
                      const void *data, int usage)
{
    ...
    int ret = ngli_gpu_buffer_init(buffer, size, NGLI_GPU_BUFFER_USAGE_TRANSFER_DST_BIT | usage);
    if (ret < 0)
        return ret;

    ret = ngli_gpu_buffer_upload(buffer, data, layout->offset, size);
    if (ret < 0)
        return ret;
...
static int gen_vec3(struct geometry *s,
                    struct gpu_buffer **bufferp, struct buffer_layout *layout,
                    size_t count, const float *data)
{
    const int format = NGLI_GPU_FORMAT_R32G32B32_SFLOAT;
    *layout = (struct buffer_layout){
        .type   = NGLI_TYPE_VEC3,
        .format = format,
        .stride = ngli_gpu_format_get_bytes_per_pixel(format),
        .comp   = ngli_gpu_format_get_nb_comp(format),
        .count  = count,
        .offset = 0,
    };
    return gen_buffer(s, bufferp, layout, data, NGLI_GPU_BUFFER_USAGE_VERTEX_BUFFER_BIT);
}
...
static int gen_vec2(struct geometry *s,
                    ...
                    size_t count, const float *data)
{
    const int format = NGLI_GPU_FORMAT_R32G32_SFLOAT;
    *layout = (struct buffer_layout){
        .type   = NGLI_TYPE_VEC2,
        .format = format,
        .stride = ngli_gpu_format_get_bytes_per_pixel(format),
...
int ngli_geometry_set_indices(struct geometry *s, size_t count, const uint16_t *indices)
{
    ...
    const int format = NGLI_GPU_FORMAT_R16_UNORM;
    ...
    return gen_buffer(s, &s->indices_buffer, &s->indices_layout, indices, NGLI_GPU_BUFFER_USAGE_INDEX_BUFFER_BIT);
}
...
void ngli_geometry_set_indices_buffer(struct geometry *s, struct gpu_buffer *buffer,
...
void ngli_geometry_freep(struct geometry **sp)
{
    ...
    if (s->buffer_ownership & OWN_VERTICES)
        ngli_gpu_buffer_freep(&s->vertices_buffer);
    if (s->buffer_ownership & OWN_UVCOORDS)
        ngli_gpu_buffer_freep(&s->uvcoords_buffer);
    if (s->buffer_ownership & OWN_NORMALS)
        ngli_gpu_buffer_freep(&s->normals_buffer);
    if (s->buffer_ownership & OWN_INDICES)
        ngli_gpu_buffer_freep(&s->indices_buffer);
    ngli_freep(&s);
}
