...
static int textureview_init(struct ngl_node *node)
{
    ...
    if (o->texture->cls->id == NGL_NODE_TEXTURE2DARRAY && o->layer >= texture_params->depth) {
        LOG(ERROR, "requested layer (%u) exceeds texture 2D array layer count (%u)", o->layer, texture_params->depth);
        return NGL_ERROR_INVALID_ARG;
    }

    if (o->texture->cls->id == NGL_NODE_TEXTURECUBE && o->layer >= 6) {
        LOG(ERROR, "requested layer (%u) exceeds cube map layer count (6)", o->layer);
        return NGL_ERROR_INVALID_ARG;
    }

    if (o->texture->cls->id == NGL_NODE_TEXTURE3D && o->layer >= texture_params->depth) {
        LOG(ERROR, "requested layer (%u) exceeds texture 3D layer count (%u)", o->layer, texture_params->depth);
        return NGL_ERROR_INVALID_ARG;
    }
