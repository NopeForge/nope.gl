                ...
                const char *base_name, const char *base_fragment)
{
    ...
    if (!o->geometry) {
        ...
        int ret = ngli_geometry_set_vertices(s->geometry, 4, default_vertices);
        if (ret < 0)
            return ret;

        ret = ngli_geometry_set_uvcoords(s->geometry, 4, default_uvcoords);
        if (ret < 0)
            return ret;

        ret = ngli_geometry_init(s->geometry, NGLI_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP);
        if (ret < 0)
            return ret;
