...
struct fgblur_priv {
    ...
    struct gpu_block down_up_data_block;
...
static int setup_interpolate_pipeline(struct ngl_node *node)
{
    ...
    const struct gpu_block_field interpolate_block_fields[] = {
        NGLI_GPU_BLOCK_FIELD(struct interpolate_block, lod, NGLI_TYPE_F32, 0),
    };
    const struct gpu_block_params interpolate_block_params = {
        .fields    = interpolate_block_fields,
        .nb_fields = NGLI_ARRAY_NB(interpolate_block_fields),
    };
    int ret = ngli_gpu_block_init(gpu_ctx, &s->interpolate.block, &interpolate_block_params);
    if (ret < 0)
        return ret;
    ...
    const struct pipeline_compat_params params = {
        ...
        .layout       = ngli_pgcraft_get_pipeline_layout(s->interpolate.crafter),
...
static int fgblur_init(struct ngl_node *node)
{
    ...
    s->mip_layout.colors[s->mip_layout.nb_colors].format = src_priv->params.format;
    ...
    int ret = ngli_gpu_block_init(gpu_ctx, &s->down_up_data_block, &down_up_data_block_params);
    if (ret < 0)
        return ret;

    ret = ngli_gpu_block_update(&s->down_up_data_block, 0, &(struct down_up_data_block){.offset=1.f});
    if (ret < 0)
        return ret;
...
static int resize(struct ngl_node *node)
{
    ...
    ngli_assert(src_priv->params.format == s->mip_layout.colors[0].format);
                                 ...
                                 struct texture *texture)
{
    ...
    ngli_pipeline_compat_update_texture(pipeline, 0, texture);
...
static void fgblur_draw(struct ngl_node *node)
{
    ...
    struct texture_priv *src_priv = (struct texture_priv *)o->source->priv_data;
    const struct image *src_image = &src_priv->image;
    ...
    for (int32_t i = 2; i <= lod_i + 1; i++)
        execute_down_up_pass(ctx, s->mips[i], s->dws.pl, ngli_rtt_get_texture(s->mips[i - 1], 0));
    ...
    if (lod_i > 0) {
        for (int32_t i = lod_i - 1; i > 0; i--)
            execute_down_up_pass(ctx, s->mips[i], s->ups.pl, ngli_rtt_get_texture(s->mips[i + 1], 0));
        execute_down_up_pass(ctx, s->mip, s->ups.pl, ngli_rtt_get_texture(s->mips[1], 0));
        mip = ngli_rtt_get_texture(s->mip, 0);
    }
    ...
    ngli_gpu_block_update(&s->interpolate.block, 0, &interpolate_block);
    ...
    ngli_gpu_ctx_begin_render_pass(ctx->gpu_ctx, ctx->current_rendertarget);
    ctx->render_pass_started = 1;
    ngli_pipeline_compat_update_texture(s->interpolate.pl, 0, mip);
    ngli_pipeline_compat_update_texture(s->interpolate.pl, 1, ngli_rtt_get_texture(s->mips[0], 0));
