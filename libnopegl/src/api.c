...
NGLI_STATIC_ASSERT("32-bit loglevel enum", sizeof(enum ngl_log_level) == sizeof(int32_t));
NGLI_STATIC_ASSERT("32-bit platform enum", sizeof(enum ngl_platform_type) == sizeof(int32_t));
NGLI_STATIC_ASSERT("32-bit backend enum",  sizeof(enum ngl_backend_type) == sizeof(int32_t));
NGLI_STATIC_ASSERT("32-bit capture enum",  sizeof(enum ngl_capture_buffer_type) == sizeof(int32_t));
...
#else
    NGLI_STATIC_ASSERT("no default platform", 0);
#endif
}
