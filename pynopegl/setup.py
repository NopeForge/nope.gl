...
class LibNopeGLConfig:
    PKG_LIB_NAME = "libnopegl"

    def __init__(self, pkg_config_bin="pkg-config"):
        import subprocess

        if subprocess.call([pkg_config_bin, "--exists", self.PKG_LIB_NAME]) != 0:
            raise Exception(f"{self.PKG_LIB_NAME} is required to build pynopegl")
...
class _WrapperGenerator:
    ...
    def _declare_class(self, class_name, params):
        ...
        # top:
        # - _pynopegl._Node: the native Cython node
        # - pynopegl.Node (from node.py): contains Python shared code
        # - pynopegl._CommonNode: generated according to the parameters of
        #   "_Node" in the nodes.specs
        # - pynopegl._***: generated and optional, these are intermediate
        #   inherited nodes (for example _AnimatedBuffer, which contains all
        #   the parameters shared by AnimatedBuffer* nodes)
        # - pynopegl.***: leaf node, accessed by the users; may contain
...
setup(
    ...
    packages=find_packages(include=["pynopegl"]),
    setup_requires=[
        "setuptools>=18.0",
        "cython>=0.29.6",
    ],
