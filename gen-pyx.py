...
get_struct_name = lambda name: 'ngl_%s' % name.lower()

def get_vec_init_code(n, vecname, cvecname):
    return '''
    ...
    for item in specs:
        ...
        for field in fields.get('constructors', []):
            ...
            if field_type in ('int', 'float', 'double'):
                ...
                special_inits += get_vec_init_code(n, field_name, cparam)
...
''' % field_data
                ...
                field_data = {
                    'field_name': field_name,
                    'cparam': cparam,
                    'vec_init_code': get_vec_init_code(n, field_name, cparam),
                }
                class_str += '''
...
if __name__ == '__main__':
    specs = yaml.load(open(sys.argv[1]))
    ofilename = sys.argv[2]
    content = gen_pyx(specs)
    open(ofilename, 'w').write(content)
