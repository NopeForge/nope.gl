...
-------------|-------|-------------------|------------
`Texture2D`, `Texture2DArray`, `Texture3D` |`mat4` | `%s_coord_matrix` | uv transformation matrix of the texture associated with the draw node using key `%s`, it should be applied to the geometry uv coordinates `ngl_uvcoord` to obtain the final texture coordinates. This variable is always exposed in the `vertex` stage.
`Texture2D`                                |`vec2` | `%s_dimensions`   | dimensions in pixels of the texture associated with the draw node using key `%s`. This variable is exposed in the same stage as the texture.
